"""FastAPI worker app — SGLang variant."""
...
from infer_worker_sglang.engine import Engine, MockSGLangEngine
from infer_worker_sglang.heartbeat import HeartbeatPusher, build_heartbeat
...
def build_app(
    ...
    engine_impl: Engine = engine or MockSGLangEngine(model_id=config.model_id)
    state: dict[str, Any] = {
        "requests_total": 0,
        "active_requests": 0,
        "last_receipt": None,
    ...
    app = FastAPI(title="infer-worker-sglang", version="0.1.0", lifespan=lifespan)
    ...
    async def chat_completions(req: ChatRequest) -> ChatResponse:
        ...
        # H-01: reject replayed customer_nonce within the LRU window.
