...
from infer_worker_sglang.heartbeat import _gateway_auth_headers, build_heartbeat
...
def test_replay_endpoint_recomputes_response_hash(config: WorkerConfig) -> None:
    ...
    expected = hashlib.sha256(
        MockSGLangEngine(config.model_id).generate(prompt, max_tokens=8, seed=0).text.encode()
    ).hexdigest()
    with TestClient(app) as client:
        r = client.post(
            "/v1/replay",
